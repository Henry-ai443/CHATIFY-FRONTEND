...
function ChatContainer() {
  const { selectedUser, messages, getMessages, sendMessage, isSendingMessage, onlineUsers, emitTyping, emitStoppedTyping, typingUsers } = useChatStore();
  ...
  useEffect(() => {
    return () => {
      // Cleanup: emit stopped typing on unmount
                  ...
                  <div>
                    <div
                      className={`rounded-lg px-3 py-2 break-words ${
                        isOwnMessage
                          ? 'bg-cyan-500/20 border border-cyan-500/50 text-slate-100'
            ...
            <div className='bg-slate-700/50 border border-slate-600/30 rounded-lg px-3 py-2'>
              <div className='flex gap-1 items-center'>
                ...
                <div className='flex gap-1'>
                  <div className='size-1.5 bg-slate-400 rounded-full animate-bounce' style={{ animationDelay: '0s' }}></div>
